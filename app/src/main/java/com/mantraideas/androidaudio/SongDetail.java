...
public class SongDetail {
	...
	public static List<SongDetail> getMp3List(String string) {
		try {
			return getMp3List(new JSONArray(string));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static List<SongDetail> getMp3List(JSONArray jArr) {
		List<SongDetail> list = new ArrayList<>();
		try {
			for (int i = 0; i < jArr.length(); i++) {
				list.add(new SongDetail(jArr.optJSONObject(i)));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public SongDetail(JSONObject jObj) {
		this.title = jObj.optString("title");
		Utilities.log(jObj.optString("title"));
		this.path = jObj.optString("path").replaceAll(" ","%20");
		this.id = 1;
		this.album_id =1;
		this.artist = "";
		this.display_name = "";
